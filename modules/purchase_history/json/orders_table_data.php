...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
    ...
    try {
        ...
        $columns = ['purchase_order_id', 'purchase_order_number', 'purchase_order_date', 'vendor_company_name', 'grand_total', 'purchase_order_status'];

        // Get the column name to sort
        $sort_column = isset($columns[$order_column]) ? $columns[$order_column] : 'purchase_order_id';
        ...
        $response = callProcedure('fetch_order_table_data', $inputParams);
        ...
        if ($response['status'] == 'success') {
            ...
            foreach ($data as $row) {
                ...
                if ($row['purchase_order_status'] == 0) {
                    ...
                    $action_buttons = <<<HTML
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#7D8ABC" class="table-action" onclick="view_purchase_order({$row['purchase_order_id']})">
                            <path d="M480-320q75 0 127.5-52.5T660-500q0-75-52.5-127.5T480-680q-75 0-127.5 52.5T300-500q0 75 52.5 127.5T480-320Zm0-72q-45 0-76.5-31.5T372-500q0-45 31.5-76.5T480-608q45 0 76.5 31.5T588-500q0 45-31.5 76.5T480-392Zm0 192q-146 0-266-81.5T40-500q54-137 174-218.5T480-800q146 0 266 81.5T920-500q-54 137-174 218.5T480-200Z" />
                        </svg>
                    
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#7D8ABC" class="table-action" onclick="edit_purchase_order({$row['purchase_order_id']})">
                            <path d="M280-160v-80h400v80H280Zm160-160v-327L336-544l-56-56 200-200 200 200-56 56-104-103v327h-80Z" />
                        </svg>
                    
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#7D8ABC" class="table-action" onclick="cancel_purchase_order({$row['purchase_order_id']})">
                            <path d="M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm80-160h80v-360h-80v360Zm160 0h80v-360h-80v360Z" />
                        </svg>
                    HTML;
                } elseif ($row['purchase_order_status'] == 1 || $row['purchase_order_status'] == 2) {
                    $status_label = $row['purchase_order_status'] == 1
                        ? '<span class="badge badge-success">Purchased</span>'
                        : '<span class="badge badge-alert">Canceled</span>';

                    // Only display the view icon for 'Purchased' and 'Canceled' statuses
                    $action_buttons = <<<HTML
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#7D8ABC" class="table-action" onclick="view_purchase_order({$row['purchase_order_id']})">
                            <path d="M480-320q75 0 127.5-52.5T660-500q0-75-52.5-127.5T480-680q-75 0-127.5 52.5T300-500q0 75 52.5 127.5T480-320Zm0-72q-45 0-76.5-31.5T372-500q0-45 31.5-76.5T480-608q45 0 76.5 31.5T588-500q0 45-31.5 76.5T480-392Zm0 192q-146 0-266-81.5T40-500q54-137 174-218.5T480-800q146 0 266 81.5T920-500q-54 137-174 218.5T480-200Z" />
                        </svg>
                ...
                $table_data[] = [
                    ...
                    'purchase_order_date' => $row['purchase_order_date'], // Already formatted in SQL
