...
--
-- Host: 127.0.0.1
-- Generation Time: Mar 06, 2025 at 06:31 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_employee` (IN `p_employee_account_id` INT)   BEGIN
    UPDATE inv_employee_accounts
    SET deleted = 1
    WHERE employee_account_id = p_employee_account_id;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `fetch_all_roles` ()   BEGIN
    ...
    WHERE 
    role_permission_status=1 AND
        deleted = 0; -- Assuming you want to exclude deleted roles
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `fetch_employee_card_details` ()   BEGIN
    DECLARE total_employees INT;
    DECLARE total_active_employees INT;
    DECLARE total_inactive_employees INT;
    ...
    FROM inv_employee_accounts
    WHERE employee_status = 1 AND deleted = 0;

    -- Calculate the total number of inactive employees
    SELECT COUNT(*) INTO total_inactive_employees
    FROM inv_employee_accounts
    WHERE employee_status = 0 AND deleted = 0;

    -- Return the results as a single result set
    SELECT 
        total_employees AS TotalEmployees,
        total_active_employees AS TotalActiveEmployees,
        total_inactive_employees AS TotalInactiveEmployees;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `fetch_employee_table_data` (IN `p_search_value` VARCHAR(255), IN `p_sort_column` VARCHAR(255), IN `p_order_dir` VARCHAR(4), IN `p_start` INT, IN `p_length` INT)   BEGIN
    DECLARE total_records INT DEFAULT 0;
    DECLARE filtered_records INT DEFAULT 0;

    -- Validate sort column
    IF p_sort_column NOT IN ('employee_id', 'employee_name', 'employee_email_id', 
                             'employee_contact_number', 'employee_designation', 
                             'employee_status') THEN
        SET p_sort_column = 'employee_id'; -- Default column
    END IF;

    -- Validate order direction
    IF p_order_dir NOT IN ('asc', 'desc') THEN
        SET p_order_dir = 'asc'; -- Default order
    END IF;
    ...
    SET @query = '
        SELECT COUNT(*) 
        FROM inv_employee_accounts e
        JOIN inv_role_permissions r ON e.employee_role_id = r.role_id 
        WHERE (e.employee_id LIKE ? 
        OR e.employee_name LIKE ? 
        OR e.employee_email_id LIKE ? 
        OR e.employee_contact_number LIKE ?)
        AND e.deleted = 0 
        AND r.role_permission_status = 1'; -- Only active roles
    
    PREPARE stmt FROM @query;
    SET @search = CONCAT('%', IFNULL(p_search_value, ''), '%');
    ...
    SET @data_query = CONCAT(
        'SELECT e.employee_account_id, e.employee_id, e.employee_name, 
        e.employee_email_id, e.employee_contact_number, 
        e.employee_designation, e.employee_status, 
        r.role_id, r.role_name AS employee_role
        FROM inv_employee_accounts e
        JOIN inv_role_permissions r ON e.employee_role_id = r.role_id 
        WHERE (e.employee_id LIKE ? 
        OR e.employee_name LIKE ? 
        ...
        OR e.employee_contact_number LIKE ?)
        AND e.deleted = 0 
        AND r.role_permission_status = 1 
        ORDER BY ', p_sort_column, ' ', p_order_dir, ' 
        LIMIT ?, ?');
    ...
    SET @start = p_start;
    SET @length = p_length;
    EXECUTE data_stmt USING @search, @search, @search, @search, @start, @length;
    DEALLOCATE PREPARE data_stmt;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `fetch_view_employee_details` (IN `employee_id` INT)   BEGIN
    SELECT 
        e.employee_account_id, 
        e.employee_id, 
        e.employee_name, 
        e.employee_dob, 
        e.employee_emergency_contact_number, 
        e.employee_email_id, 
        e.employee_street, 
        e.employee_locality, 
        e.employee_district, 
        e.employee_state, 
        e.employee_pincode, 
        e.employee_joined_date, 
        e.employee_contact_number, 
        e.employee_designation, 
        e.employee_role_id, 
        r.role_name, -- Fetch role name from the joined table
        e.employee_username, 
        e.employee_pic, 
        e.employee_status, 
        e.deleted
    FROM 
        inv_employee_accounts e
    LEFT JOIN 
        inv_role_permissions r ON e.employee_role_id = r.role_id
    WHERE 
        e.employee_account_id = employee_id
    AND 
    	e.employee_status=1
    AND
        e.deleted = 0; -- Ensures only non-deleted records are fetched
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `generate_new_employee_number` ()   BEGIN
    DECLARE last_number INT;

    -- Fetch the last employee number and extract the numeric part
    SELECT 
        CAST(SUBSTRING_INDEX(employee_id, '-', -1) AS UNSIGNED) 
    INTO last_number
    FROM inv_employee_accounts
    ORDER BY created_on DESC
    LIMIT 1;

    -- If no employee number exists, set the number to 0
    IF last_number IS NULL THEN
        SET last_number = 0;
    END IF;

    -- Generate the new employee number with EM- prefix
    SELECT CONCAT('EM-', LPAD(last_number + 1, 3, '0')) AS new_employee_number;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_employee` (IN `p_employee_name` VARCHAR(255), IN `p_employee_dob` DATE, IN `p_employee_photo` VARCHAR(255), IN `p_employee_contact_number` VARCHAR(12), IN `p_employee_emergency_contact_number` VARCHAR(12), IN `p_employee_email_id` VARCHAR(255), IN `p_employee_street` VARCHAR(255), IN `p_employee_locality` VARCHAR(255), IN `p_employee_district` VARCHAR(255), IN `p_employee_state` VARCHAR(255), IN `p_employee_pincode` VARCHAR(10), IN `p_employee_joined_date` DATE, IN `p_employee_designation` VARCHAR(100), IN `p_employee_role_id` INT, IN `p_employee_username` VARCHAR(255), IN `p_employee_password` TEXT, IN `p_created_by` INT)   BEGIN
    DECLARE new_employee_id VARCHAR(20);

    -- Generate a new employee ID (you can adjust this logic as needed)
    SET new_employee_id = CONCAT('EMP-', LPAD((SELECT COALESCE(MAX(employee_account_id), 0) + 1 FROM inv_employee_accounts), 4, '0'));

    -- Insert the employee data into the table
    INSERT INTO inv_employee_accounts (
        employee_id,
        employee_name,
        employee_dob,
        ...
        p_employee_emergency_contact_number,
        p_employee_email_id,
        p_employee_street,
        p_employee_locality,
        p_employee_district,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_employee_account` (IN `p_employee_account_id` INT, IN `p_employee_name` VARCHAR(255), IN `p_employee_dob` DATE, IN `p_employee_photo` VARCHAR(255), IN `p_employee_contact_number` VARCHAR(12), IN `p_employee_emergency_contact_number` VARCHAR(12), IN `p_employee_email_id` VARCHAR(255), IN `p_employee_street` VARCHAR(255), IN `p_employee_locality` VARCHAR(255), IN `p_employee_district` VARCHAR(255), IN `p_employee_state` VARCHAR(255), IN `p_employee_pincode` VARCHAR(10), IN `p_employee_joined_date` DATE, IN `p_employee_designation` VARCHAR(100), IN `p_employee_role_id` INT)   BEGIN
    UPDATE inv_employee_accounts
    SET
        employee_name = p_employee_name,
        employee_dob = p_employee_dob,
        employee_contact_number = p_employee_contact_number,
        employee_emergency_contact_number = p_employee_emergency_contact_number,
        employee_email_id = p_employee_email_id,
        employee_street = p_employee_street,
        employee_locality = p_employee_locality,
        employee_district = p_employee_district,
        employee_state = p_employee_state,
        employee_pincode = p_employee_pincode,
        employee_joined_date = p_employee_joined_date,
        employee_designation = p_employee_designation,
        employee_role_id = p_employee_role_id,
        employee_pic = p_employee_photo,
        updated_on = CURRENT_TIMESTAMP
    WHERE
        employee_account_id = p_employee_account_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `update_employee_status` (IN `p_employee_account_id` INT, IN `p_employee_status` INT)   BEGIN
    UPDATE inv_employee_accounts
    SET employee_status = p_employee_status
    WHERE employee_account_id = p_employee_account_id;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `validate_and_log_login` (IN `p_username` VARCHAR(255), IN `p_ip_address` VARCHAR(45))   BEGIN
    ...
    DECLARE v_employee_pic TEXT;
    DECLARE v_login_id INT;

    -- Check if the user exists with the provided username
    SELECT employee_account_id, employee_role_id, employee_name, employee_pic INTO v_user_id, v_role_id, v_employee_name,v_employee_pic
    FROM inv_employee_accounts
    ...
    IF v_user_id IS NOT NULL THEN
        ...
        SELECT p_username AS username, v_role_id AS role_id, v_employee_name AS employee_name, v_login_id AS login_id, v_user_id AS user_id,v_employee_pic AS employee_pic;
    ELSE
        -- User does not exist
...
CREATE TABLE `inv_employee_accounts` (
  ...
  `employee_emergency_contact_number` varchar(12) NOT NULL,
  `employee_email_id` varchar(255) NOT NULL,
  `employee_street` varchar(255) NOT NULL,
  `employee_locality` varchar(255) NOT NULL,
  `employee_district` varchar(255) NOT NULL,
...
INSERT INTO `inv_employee_accounts` (`employee_account_id`, `employee_id`, `employee_name`, `employee_dob`, `employee_emergency_contact_number`, `employee_email_id`, `employee_street`, `employee_locality`, `employee_district`, `employee_state`, `employee_pincode`, `employee_joined_date`, `employee_contact_number`, `employee_designation`, `employee_role_id`, `employee_username`, `employee_password`, `employee_pic`, `log_id`, `created_on`, `created_by`, `updated_on`, `employee_status`, `deleted`) VALUES
(5, 'EM-001', 'Ajay S', '2002-08-16', '8754857015', 'ajay@gmail.com', 'iron man street', 'stark tower', 'manhattan', 'new york', '605008', '2024-10-01', '8754857015', 'worker', 1, 'ajay', '$argon2id$v=19$m=65536,t=4,p=1$OWc5eWkvQzgvTjkxdHVtUw$IirK6bD+SzRbTBiCOaaoKvcG0kcmV+vTeUbpWGOnPeM', 'employee_photo_EM-001black_widow_in_captain_america_the_winter_soldier-wallpaper-1920x1080.jpg', 49, '2024-10-18 09:53:02', 1, '2024-11-29 11:13:18', 1, 0),
(13, 'EMP-0006', 'Stacey Camacho', '0000-00-00', '8754857015', 'sysy@mailinator.com', 'In voluptates veniam', 'Aut sapiente deserun', 'Sed ut nesciunt vel', 'Expedita vero nulla', 'Quisquam q', '1972-04-09', '8754857015', 'Laboris non nisi aut', 1, 'guhakupe', '$argon2id$v=19$m=65536,t=4,p=1$aDBFRlBaOHpQMXpuV2ZDUw$NCHXCxLV//BVnRHYxWk9eyFblvS3JZy4Nq55wdRdIZM', '', 0, '2024-10-18 12:01:36', 5, '2024-10-18 17:29:49', 1, 1),
(21, 'EMP-0014', 'Cameron Kent', '1994-06-20', '9629862944', 'zypelu@mailinator.com', 'Eius asperiores ut a', 'Molestiae dolores ma', 'Dolor assumenda sit', 'Qui voluptas perfere', 'Nulla sunt', '1982-06-14', '9629862944', 'Nesciunt incidunt', 1, 'summa2', '$argon2id$v=19$m=65536,t=4,p=1$bERteTZvaDAuMU1lcU9nTA$cdFyov6VI84TWI9pDOEaXBXF6/u8Ja9d3BNHgvsUasw', 'employee_photo_EM-007pirates_of_the_caribbean_on_stranger_tides_2011___johnny_depp_as_captain_ja.jpg', 0, '2024-10-18 16:58:24', 5, '2024-10-18 17:33:22', 1, 0),
(23, 'EMP-0022', 'Regina Russell', '1992-03-16', '9629862944', 'xudog@mailinator.com', 'Consectetur minima e', 'Voluptatem Tempore', 'Voluptatem Voluptas', 'Tempore voluptas ex', 'Doloribus', '2001-10-15', '9629862944', 'Mollit in consectetu', 1, 'wupedesy', '$argon2id$v=19$m=65536,t=4,p=1$RHFSRm02aFUzUnRHVi9qYw$+ZhdE4LNmWiE+gz6rguc3sezUtui7u2+eci2DM+RdcA', 'employee_photo_EMP-0022black_widow_in_captain_america_the_winter_soldier-wallpaper-1920x1080.jpg', 0, '2024-10-18 16:59:51', 5, '2024-10-18 18:04:18', 1, 0),
(24, 'EMP-0024', 'Kylee Shelton', '2003-04-08', '3210654987', 'guxihekol@mailinator.com', 'Qui laborum occaecat', 'Amet quia omnis adi', 'Id est ut voluptas u', 'Officia culpa sed t', 'Voluptatem', '2012-05-13', '3210654987', 'Laborum error ea eu', 1, 'daqad', '$argon2id$v=19$m=65536,t=4,p=1$YkNuZTM0TWRMbWRTbXFkUw$blq4dBzJ2F4FHMR38LWf7O9H+MUjeYzl9wBSiF8kH0A', 'employee_photo_EM-023assassins_creed_odyssey-wallpaper-1920x1080.jpg', 0, '2024-10-18 17:10:08', 5, '2024-10-18 17:10:08', 1, 0);
...
(16, 'INV-009', 1, '2024-10-06 03:08:54', '2024-10-04 18:30:00', 32.10, 1, 0.00, 0.00, 0.00, 0.00, 32.10, 0.00, 32.10, 'Thirty Two And Ten Paise Only', '', '2024-10-04 13:18:32', '2024-10-04 13:18:32', 4, 1, 0),
(17, 'INV-010', 1, '2024-10-05 18:30:00', '2024-10-04 18:30:00', 90000.00, 1, 0.00, 0.00, 0.00, 0.00, 90000.00, 0.00, 90000.00, 'Ninety Thousand Only', '', '2024-10-04 13:18:45', '2024-10-04 13:18:45', 4, 0, 0),
(18, 'INV-011', 1, '2024-10-06 18:30:00', '2024-10-07 18:30:00', 72112250.64, 1, 0.00, 0.00, 0.00, 0.00, 72112250.64, -0.74, 68506637.37, 'Six Crore And Eighty Five Lakh And Six Thousand And Six Hundred And Thirty Eight Only', 'UPI', '2024-10-07 13:00:46', '2024-10-07 13:01:17', 4, 1, 0);
...
(38, 4, '::1', '2024-10-07 12:55:05', NULL, 0),
(39, 4, '::1', '2024-10-18 09:06:40', NULL, 1),
(40, 5, '::1', '2024-10-18 09:53:29', NULL, 0),
(41, 5, '::1', '2024-10-18 13:28:00', NULL, 0),
(42, 5, '::1', '2024-10-18 16:38:28', NULL, 0),
(43, 5, '::1', '2024-10-18 18:09:16', NULL, 0),
(44, 5, '::1', '2024-10-18 18:09:58', NULL, 0),
(45, 5, '::1', '2024-10-18 18:10:13', NULL, 0),
(46, 5, '127.0.0.1', '2024-10-18 18:11:00', NULL, 0),
(47, 5, '::1', '2024-10-18 18:20:14', NULL, 0),
(48, 5, '::1', '2024-10-19 14:04:50', NULL, 0),
(49, 5, '::1', '2024-11-29 11:13:18', NULL, 1);
...
(2, 'V-002', 'Messrs.', 'Google', 'Bill Gates', '5647893210', 'gates@gmail.com', 'No.18, Rose Street', 'Ashok Nagar', 'Lawspet', 'Pondicherry', 'India', 'No.18, Rose Street', 'Ashok Nagar', 'Lawspet', 'Pondicherry', 'Pondicherry', '605008', 'India', 'Pondicherry', '605008', '7856HJK8825', 'PAN47922', 'ICICI Bank', '688765143554', 'ICI3466', 'ECR Branch', '2024-09-13 00:33:21', '2024-10-19 14:30:37', 4, 1, 0);
...
--
ALTER TABLE `inv_employee_accounts`
  MODIFY `employee_account_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=25;
...
--
ALTER TABLE `inv_login_logs`
  MODIFY `login_log_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=50;
