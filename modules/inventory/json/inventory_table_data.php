...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
    try {
        ...
        $columns = ['product_name', 'product_code', 'unit_of_measure', 'quantity_in_stock', 'last_updated'];
        ...
        $response = callProcedure('fetch_inventory_table_data', $inputParams);
        if ($response['status'] == 'success') {
            ...
            $s_no = $start + 1;
            foreach ($data as $row) {
                $row['updated_on'] = date('d-m-Y', strtotime($row['updated_on']));
                $action_buttons = <<<HTML
                <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="30px" fill="#7D8ABC" class="table-action" onclick="view_inventory({$row['inventory_id']})">
                    <path d="M480-320q75 0 127.5-52.5T660-500q0-75-52.5-127.5T480-680q-75 0-127.5 52.5T300-500q0 75 52.5 127.5T480-320Zm0-72q-45 0-76.5-31.5T372-500q0-45 31.5-76.5T480-608q45 0 76.5 31.5T588-500q0 45-31.5 76.5T480-392Zm0 192q-146 0-266-81.5T40-500q54-137 174-218.5T480-800q146 0 266 81.5T920-500q-54 137-174 218.5T480-200Z" />
                </svg>

                <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="30px" fill="#7D8ABC" class="table-action" onclick="update_inventory({$row['inventory_id']},{$row['product_id']},'{$row['product_name']}','{$row['unit_of_measure']}',{$row['quantity_in_stock']})">
