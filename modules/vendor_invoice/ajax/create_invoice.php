...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
    ...
    $handling_fees_amount = isset($_POST['handling_fees_amount']) ? sanitizeInput($_POST['handling_fees_amount'], 'float') : 0.00;
    $storage_fees = isset($_POST['storage_fees']) ? sanitizeInput($_POST['storage_fees'], 'float') : 0.00;
    $shipping_charges = isset($_POST['shipping_charges']) ? sanitizeInput($_POST['shipping_charges'], 'float') : 0.00;
    ...
    // Validate input
    $errors = [];
    if (empty($vendor_id)) $errors[] = "Customer ID is required.";
    ...
    $procedure_params = [
        ...
        ['value' => $cgst, 'type' => 'd'],
        ['value' => $igst, 'type' => 'd'],
        ['value' => $shipping_charges, 'type' => 'd'],
        ['value' => $handling_fees_amount, 'type' => 'd'],
        ['value' => $storage_fees, 'type' => 'd'],
    ...
    try {
        ...
        $result = callProcedure('insert_vendor_invoice_with_items', $procedure_params);
