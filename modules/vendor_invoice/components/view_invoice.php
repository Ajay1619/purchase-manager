            ...
            <div class="view-container">
                    ...
                    <div class="flex-container">
                        <div class="flex-row">
                            <div class="flex-column">
                                ...
                                <span class="value" id="vendor-name">vendor A</span>
                            </div>
                            <div class="flex-column">
                ...
                <div class="section">
                    ...
                    <div class="flex-row">
                        ...
                        <div class="flex-column">
                            <label for="igst">Shipping Charges Amount ( <?= CURRENCY_SYMBOL ?> ):</label>
                            <span class="value" id="shipping-charges">0.00</span>
                        </div>
                        <div class="flex-column">
                            <label for="adjustment">Handling Fees ( <?= CURRENCY_SYMBOL ?> ):</label>
                            <span class="value" id="handling-fees">0.00</span>
                        </div>
                    </div>
                    <div class="flex-row">

                        <div class="flex-column">
                            <label for="grand-total">Storage Fees Total ( <?= CURRENCY_SYMBOL ?> ):</label>
                            <span class="value" id="storage-fees">2400.00</span>
                        </div>
                        <div class="flex-column">
        ...
        <script>
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#vendor-name').text(invoiceDetails.vendor_company_name);
                    $('#invoice-number').text(invoiceDetails.vendor_invoice_number); // Match the field name with your response
                    $('#in-code').val(invoiceDetails.vendor_invoice_number); // Hidden input to hold invoice number
                    $('#invoice-date').text(invoiceDetails.invoice_date);
                    $('#due-date').text(invoiceDetails.invoice_due_date);

                    // Billing Address
                    $('#billing-address-street').text(invoiceDetails.billing_address_street);
                    $('#billing-address-locality').text(invoiceDetails.billing_address_locality);
                    $('#billing-address-district').text(invoiceDetails.billing_address_district);
                    $('#billing-address-state').text(invoiceDetails.billing_address_state);
                    $('#billing-address-pincode').text(invoiceDetails.billing_address_pincode);

                    // Shipping Address
                    $('#shipping-address-street').text(invoiceDetails.billing_address_street); // Assuming the same as billing address
                    $('#shipping-address-locality').text(invoiceDetails.billing_address_locality);
                    $('#shipping-address-district').text(invoiceDetails.billing_address_district);
                    $('#shipping-address-state').text(invoiceDetails.billing_address_state);
                    $('#shipping-address-pincode').text(invoiceDetails.billing_address_pincode);
                    ...
                    $('#shipping-charges').text(invoiceDetails.shipping_charges);
                    $('#handling-fees').text(invoiceDetails.handling_fees);
                    $('#storage-fees').text(invoiceDetails.storage_fees);
            ...
            $('#export-invoice').on('click', function() {
                $.ajax({
                    url: '<?= MODULES . '/vendor_invoice/ajax/invoice_pdf.php' ?>',
                    type: 'POST',
                    data: {
                        in_id: <?= $invoice_id ?>,
                        in_code: $('#in-code').val()
