...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
    ...
    try {
        ...
        $columns = ['invoice_number', 'invoice_date', 'customer_name', 'invoice_status', 'grand_total'];

        // Get the column name to sort
        $sort_column = isset($columns[$order_column]) ? $columns[$order_column] : 'invoice_id';
        ...
        $response = callProcedure('fetch_invoice_table_data', $inputParams);
        ...
        if ($response['status'] == 'success') {
            ...
            foreach ($data as $row) {
                ...
                // Prepare action buttons with invoice_id parameter (sample actions)
                if ($row['invoice_status'] == 0) {
                    $status_label = '<span class="badge badge-warning">Pending</span>';
                    // Display all icons for 'Pending' status
                    $action_buttons = <<<HTML
                        <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="30px" fill="#7D8ABC" class="table-action" onclick="view_invoice({$row['invoice_id']})">
                            <path d="M480-320q75 0 127.5-52.5T660-500q0-75-52.5-127.5T480-680q-75 0-127.5 52.5T300-500q0 75 52.5 127.5T480-320Zm0-72q-45 0-76.5-31.5T372-500q0-45 31.5-76.5T480-608q45 0 76.5 31.5T588-500q0 45-31.5 76.5T480-392Zm0 192q-146 0-266-81.5T40-500q54-137 174-218.5T480-800q146 0 266 81.5T920-500q-54 137-174 218.5T480-200Z" />
                        </svg>
                    
                        <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="30px" fill="#7D8ABC" class="table-action" onclick="edit_invoice({$row['invoice_id']})">
                            <path d="M280-160v-80h400v80H280Zm160-160v-327L336-544l-56-56 200-200 200 200-56 56-104-103v327h-80Z" />
                        </svg>
                    
                        <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="30px" fill="#7D8ABC" class="table-action" onclick="cancel_invoice({$row['invoice_id']})">
