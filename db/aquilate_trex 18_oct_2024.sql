...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 18, 2024 at 08:11 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `fetch_vendor_table_data` (IN `p_start` INT, IN `p_length` INT, IN `p_search_value` VARCHAR(255), IN `p_sort_column` VARCHAR(50), IN `p_order_dir` VARCHAR(4))   BEGIN
    ...
    -- Validate sort column
    IF p_sort_column NOT IN ('employee_id', 'employee_name', 'employee_email_id', 'employee_contact_number', 'employee_designation', 'employee_status') THEN
        SET p_sort_column = 'employee_id'; -- Default column
    END IF;
    ...
    SET @query = '
        ...
        OR employee_email_id LIKE ? 
        OR employee_contact_number LIKE ?)
        AND deleted = 0 ';
    PREPARE stmt FROM @query;
    SET @search = CONCAT('%', p_search_value, '%');
    EXECUTE stmt USING @search, @search, @search, @search;
    ...
    -- Fetch data with pagination and sorting
    SET @data_query = CONCAT(
        'SELECT employee_account_id, employee_id, employee_name, employee_email_id, 
        employee_contact_number, employee_designation, employee_status 
        FROM inv_employee_accounts 
        WHERE (employee_id LIKE ? 
        OR employee_name LIKE ? 
        OR employee_email_id LIKE ? 
    ...
    PREPARE data_stmt FROM @data_query;
    EXECUTE data_stmt USING @search, @search, @search, @search, p_start, p_length;
    DEALLOCATE PREPARE data_stmt;
...
(46, 5, '127.0.0.1', '2024-10-18 18:11:00', NULL, 1);
